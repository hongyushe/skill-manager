...
对话式 AI 有两个致命缺陷：要人盯着（不发消息它就不动，关掉浏览器就停了）、上下文污染（两次任务的上下文会串）。Skill-Manager 从架构层面解决这两个问题——每个任务独立上下文窗口，互不干扰；四种触发模式（Cron / Webhook / Manual / Chain，最深 5 层链式串联）；自然语言一句话建任务，LLM 自动解析为结构化配置。
...
Conversational AI has two fatal flaws: it needs you watching (no message = no action, close the browser and it stops), and context pollution (different tasks bleed into each other). Skill-Manager solves both at the architecture level — each task runs in an isolated context window; four trigger modes (Cron/Webhook/Manual/Chain, up to 5 levels deep); and natural language task creation where the LLM auto-parses your sentence into structured config.
